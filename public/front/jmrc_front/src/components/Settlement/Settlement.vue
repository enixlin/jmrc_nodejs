<template>
  <div id="settlement" class="main">
    ...
    <div id="total" v-show="this.level == '全行'">
      <!-- 左则展示区 任务完成率进度条 + 分月图 -->
        ...
        >
          ...
          <el-table-column prop="amount_c" label="金额" width="90" sortable header-align="center">
            <template style="align:right" slot-scope="scope">
              <p align="right">
                <font size="1">
                  {{
                  number_format(scope.row.amount_c / 10000, 2)
                  }}
                </font>
              </p>
            </template>
          ...
          >
            <template style="text-align:right" slot-scope="scope">
              <p align="right">
                <font v-if="scope.row.times_c - scope.row.times_p > 0" color="green" size="1">
                  {{
                  number_format(scope.row.times_c - scope.row.times_p, 0)
                  }}
                </font>
              </p>
              <p align="right">
                <font v-if="scope.row.times_c - scope.row.times_p < 0" color="red" size="1">
                  {{
                  number_format(scope.row.times_c - scope.row.times_p, 0)
                  }}
                </font>
          ...
          <el-table-column header-align="center" width="70" align="center" label="操作">
            <template slot-scope="scope">
              ...
              <i
                ...
                @click="total_product_month(scope.$index, scope.row)"
              ></i>
            </template>
          </el-table-column>
        </el-table>
    ...
    <!-- 经营单位业务 模板开始 -->
    <div id="unit" class="unitdiv" v-show="this.level == '经营单位'"></div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async query_total() {
      ...
      this.recordDate_month = this.recordDate.substring(5, 6);
      this.recordDate_day = this.recordDate.substring(7, 8);
    },
    //查询办理该项产品的所有客户
