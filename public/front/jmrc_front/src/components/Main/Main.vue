...
/* 这里要注意，引用自定义模块时，引用的路径是当前引用的文件为起点，这里是Main.vue的路径为起点   @ 等价于 /src 这个目录，避免写麻烦又易错的相对路径 */
import Utils from "@/Utils/Utils";

import menutree from "@/components/Main/MenuTree.vue";
import eventBus from "@/Utils/EventBus.js";
import useradmin from "@/components/User/Admin.vue";
import userroleradmin from "@/components/User/UserRoler.vue";
import rolerfeatureadmin from "@/components/Roler/RolerFeature.vue";
import roleradmin from "@/components/Roler/Admin.vue";
import featureadmin from "@/components/Feature/Admin.vue";
import configRangeProduct from "@/components/Settlement/ConfigRangeProduct.vue";
...
export default {
  ...
  components: {
    menutree: menutree,
    useradmin: useradmin,
    roleradmin:roleradmin,
    userroleradmin:userroleradmin,
    rolerfeatureadmin:rolerfeatureadmin,
  ...
  mounted() {
    let me = this;
    var util = new Utils();
    axios.post("/features/getFeaturesByUser",{id:this.$store.state.authUser.id}).then(result => {
      console.log("authusre.id");
